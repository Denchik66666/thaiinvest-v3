...
type InvestorDetailResponse = {
  ...
  topUpRequests: Array<{
    ...
    /** Календарная дата заявки; если нет — смотреть `createdAt`. */
...
export default function InvestorDetailPageClient({ investorId }: { investorId: string }) {
  ...
  const availableInterest = useMemo(() => {
    ...
    return Math.max(investor.accrued ?? 0, 0);
  }, [investor]);
  ...
  const bodyTopUpMutation = useMutation({
    mutationFn: async () => {
      if (!investor) throw new Error("Нет данных инвестора");
      ...
      return apiClient.post("/api/body-topup-requests", {
        ...
        requestDate: investorEntryToYmd(investor.entryDate),
      });
    },
    onSuccess: () => {
  ...
  const hasPendingBodyTopUp = useMemo(() => {
    const list = investorData?.topUpRequests ?? [];
    return list.some((r) => r.status === "pending_investor");
  }, [investorData?.topUpRequests]);
      ...
      <div className="thai-dashboard-root min-h-screen space-y-3 py-3 pb-24 md:space-y-5 md:py-8 md:pb-28">
        ...
        {isAdmin ? (
          <div className="thai-panel-admin space-y-3 md:space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {user.role === "OWNER" && !investor.isPrivate ? (
                <Button
                  type="button"
                  variant="outline"
                  disabled={hasPendingBodyTopUp}
            ...
            {credentials ? (
              ...
              <Text className="text-xs text-muted-foreground">
                Логин в системе:{" "}
                <span className="font-medium text-foreground">{investorDisplayHandle(investor) ?? "нет"}</span>
              </Text>
            )}
          </div>
        ) : null}
        ...
        {user.role === "OWNER" && !investor.isPrivate ? (
          <ManagePositionDeskModal
            mode="body_topup"
            open={bodyTopupOpen}
            onClose={() => !bodyTopUpMutation.isPending && setBodyTopupOpen(false)}
            positionLabel={investorDisplayHandle(investor) ?? investor.name}
            currentBody={investor.body}
