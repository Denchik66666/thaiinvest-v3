...
3. **`BusinessRateControlCenter`** — дожать VIP-компакт: метки 10–11px uppercase, суммы tabular-nums, одна строка ключевых цифр где уместно; журнал и «Календарь · план» остаются вторичными поверхностями.  
   **Сделано (2026-05-09):** верхняя зона — одна строка «ставка + с даты + далее»; уменьшены отступы и высоты полей (`h-9`, `text-xs` в инпуте); блок «Далее» в одну строку с разделителями; журнал/календарь — плотнее (`py-2` / `px-3`); well выбранного дня и строки плана — `10–11px` + `tabular-nums`.
