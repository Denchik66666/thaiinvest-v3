 ...
 *   Пароль не хардкодим: PLAYWRIGHT_SUPERADMIN_PASSWORD | PLAYWRIGHT_DEN_PASSWORD | PLAYWRIGHT_LOGIN_PASSWORD | SUPERADMIN_PASSWORD.
...
function superAdminPassword(): string | undefined {
  return (
...
test.describe("SUPER_ADMIN: удаление операции через модалку финансов", () => {
  test("модалка → подтверждение «Удалить» → записи нет в operations-history", async ({ browser, baseURL }) => {
    ...
    test.skip(
      !password,
      "Задайте пароль супера: PLAYWRIGHT_SUPERADMIN_PASSWORD, PLAYWRIGHT_DEN_PASSWORD, PLAYWRIGHT_LOGIN_PASSWORD или SUPERADMIN_PASSWORD в .env"
    );
