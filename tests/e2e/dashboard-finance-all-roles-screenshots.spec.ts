...
/**
 * Скриншоты главной и «Финансы» для SUPER_ADMIN (admin) / Sam / Sega (desktop dark).
...
test("dashboard + finance screenshots admin Sam Sega", async ({ page, context }) => {
  ...
  await login(context, superAdminUser, superAdminPw);
  ...
  await page.screenshot({ path: `${OUT}/02-admin-finance.png`, fullPage: true });
